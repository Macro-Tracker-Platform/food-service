...
public class FoodVoiceQuotaReservationService {
    static final String PREMIUM_PREFIX = "scans:voice:premium:";
    static final String PREMIUM_INFLIGHT_PREFIX = "scans:voice:premium:inflight:";
    static final String PREMIUM_COMPLETED_PREFIX = "scans:voice:premium:completed:";
    static final String FREE_INFLIGHT_PREFIX = "scans:voice:free:inflight:";
    ...
    public Reservation reserve(Long userId, boolean premium, String requestToken) {
        ...
        if (used == null) {
            throw new IllegalStateException("Could not reserve premium voice food scan quota");
        }
        if (used < 0) {
    ...
    private Reservation reserveFree(Long userId, String token, long now, long expiresAt) {
        FoodPhotoScanCreditDto credits = entitlementClient.getVoiceFoodScanCredits(userId);
        ...
        if (result == null) {
            throw new IllegalStateException("Could not reserve free voice food scan quota");
        }
        if (result < 0) {
            throw freeLimitReached();
        }
    ...
    public QuotaSnapshot commitSuccessfulFoodScan(Reservation reservation) {
        ...
        if (used == null) {
            throw new IllegalStateException("Could not commit premium voice food scan quota");
        }
        if (used < 0) {
            throw dailyLimitReached();
        }
    ...
    private QuotaSnapshot commitFree(Reservation reservation) {
        try {
            FoodPhotoScanCreditDto credits = entitlementClient.consumeVoiceFoodScanCredit(
                    reservation.userId(), reservation.token());
            if (credits == null) {
                throw new IllegalStateException("Could not consume voice food scan credit");
            }
    ...
    private long reservationTtlSeconds() {
        return properties.getFoodVoiceScan().getReservationTtlSeconds();
    }

    private int premiumLimit() {
        return properties.getFoodVoiceScan().getPremiumDailyLimit();
    }
    ...
    private LocalDate quotaDate() {
        return LocalDate.now(properties.getFoodVoiceScan().getQuotaZone());
    }
